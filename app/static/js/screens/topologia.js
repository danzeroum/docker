...
export function renderTopologia(container) {
  ...
  async function carregar() {
    ...
    // A corrente so e redesenhada quando ingress ou inventario mudaram — o que
    // acontece por deploy, nao a cada 30s.
    if (!redesenharSeMudou(corpo, [ing.data || null, ovw.data || null], () => desenhar(ing, ovw))) return;
  }

  function desenhar(ing, ovw) {
    const corpo = el('topoBody');
    if (!corpo) return;
